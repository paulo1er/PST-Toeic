...
def convert_from_path(pdf_path, dpi=200, output_folder=None, first_page=None, last_page=None, fmt='ppm', thread_count=1, userpw=None, use_cropbox=False, strict=False, transparent=False, output_file=str(uuid.uuid4())):
    ...
    for i in range(thread_count):
        ...
        processes.append((thread_output_file, Popen(args, stdout=PIPE, stderr=PIPE, startupinfo=startupinfo)))
    images = []
...
def _page_count(pdf_path, userpw=None):
    try:
        ...
        startupinfo.dwFlags |= subprocess.STARTF_USESHOWWINDOW
        if userpw is not None:
            proc = Popen(["pdfinfo", pdf_path, '-upw', userpw], stdout=PIPE, stderr=PIPE, startupinfo=startupinfo)
        else:
            proc = Popen(["pdfinfo", pdf_path], stdout=PIPE, stderr=PIPE, startupinfo=startupinfo)
